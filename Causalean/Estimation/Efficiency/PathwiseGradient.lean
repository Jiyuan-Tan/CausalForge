...
/-- A **regular submodel** through the base law `P₀ : 𝓛`, with tangent direction
`one`-orthogonal (mean-zero) score `score`.

It is a one-parameter family of laws `path : ℝ → 𝓛` with `path 0 = P₀`, equipped
with an L² score `score : H` satisfying the mean-zero constraint
`⟪score, one⟫ = 0`. The path and score are abstract interface fields: we do not
construct parametric families from measures. -/
...
/-- `T` is a **tangent space** for the submodels through `P₀`: it contains every
submodel score and is contained in the closed span of the score set (hence equals
that closed span). In applications this is the closed subspace of mean-zero L²
functions spanned by the available scores (all of mean-zero L² in the
nonparametric model). -/
