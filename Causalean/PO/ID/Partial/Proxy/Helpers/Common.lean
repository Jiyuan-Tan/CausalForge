...
/-- Fix a treatment arm `a` and assume [the two-proxy bridge assumption bundle](hyp:HA), with
[the treatment and outcome variables distinct](hyp:_hAY). Then, restricted to the stratum
