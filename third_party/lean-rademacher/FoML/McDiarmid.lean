...
theorem ProbabilityTheory.iIndepFun.comp_right
  ...
  have h₁ : ∀ i ∈ s, @MeasurableSet Ω (MeasurableSpace.comap (f i) (mβ i)) (f₁ i) := by
    ...
    change MeasurableSpace.comap (f i) (mβ i) =
      MeasurableSpace.comap (f (g ↑(invg ⟨i, hi⟩))) (mβ (g ↑(invg ⟨i, hi⟩)))
    rw [(invg ⟨i, hi⟩).2.2]
              ...
              · apply (hmeasurableY hX'' hf'' (⟨k, h⟩ : Fin m).succ).comp
                ...
                if h' : i.1 < k then
                  have : (fun c : Ω × Ω ↦
                    ...
                    rw [dif_pos h']
                    simp
                  exact this ▸ ((hX'' (Fin.castLE h i)).comp measurable_snd)
                else
                  have : (fun c : Ω × Ω ↦
                    @Fin.snoc k (fun _ ↦ 𝓧) (fun i : Fin k ↦ X' (Fin.castLE h (Fin.castSucc i)) c.2) (X' ⟨k, h⟩ c.1) i)
                    = fun c ↦ X' ⟨k, h⟩ c.1 := by
                    ...
                    rw [dif_neg h']
                    simp
                  exact this ▸ ((hX'' ⟨k, h⟩).comp measurable_fst)
