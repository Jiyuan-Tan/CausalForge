...
lemma general_polyhedra_is_polyhedra_empty (τ σ : Finset ℕ) (he : ¬(τ ∪ σ).Nonempty) :
  ...
  · intro x0
    rcases x0 with ⟨lam, mu, hx⟩
    have hsumτ : (∑ i ∈ τ.attach, lam i • a i) = 0 := by
      apply Finset.sum_eq_zero
      intro i _
      exfalso
      simpa [he.1] using i.2
    have hsumσ : (∑ i ∈ σ.attach, mu i • b i) = 0 := by
      apply Finset.sum_eq_zero
      intro i _
      exfalso
      simpa [he.2] using i.2
    rw [hsumτ, hsumσ] at hx
    simp at hx
    constructor
    · exact ⟨0, by simp⟩
    · simp [hx]
  · intro cond
    use (fun _ => 0)
    use (fun _ => 0)
    simpa [he.1, he.2] using cond.2.symm
...
lemma general_polyhedra_is_polyhedra_ne (τ σ : Finset ℕ) (he : (τ ∪ σ).Nonempty) :
  ...
  let μ := σ ∪ τ1 ∪ τ2
  have mt1emp : σ ∩ τ1 = ∅ := by
    simp only [τ1]; apply s_inter_t1_empty he; rfl
  have mt2emp : σ ∩ τ2 = ∅ := by
    simp only [τ2]; apply s_inter_t2_empty he; rfl
  have t1t2emp : τ1 ∩ τ2 = ∅ := by
    simp only [τ1, τ2]; apply t1_inter_t2_empty he; rfl
  ...
  · rintro ⟨lam, mu, munneg, xeq⟩
    ...
    have eq2 : ∑ x : τ, (fun y => lamp y • cτ1 y) x = ∑ x ∈ τ1, (fun y => w y • c y) x := by
      ...
      have hns : x.1 ∉ σ := by
        intro hxσ
        have hxinter : x.1 ∈ σ ∩ τ1 := Finset.mem_inter.mpr ⟨hxσ, x.2⟩
        simp [mt1emp] at hxinter
      simp [w, c, hns]
    have eq3 : ∑ x : τ, (fun y => lamn y • cτ2 y) x = ∑ x ∈ τ2, (fun y => w y • c y) x := by
      ...
      have hns : x.1 ∉ σ := by
        intro hxσ
        have hxinter : x.1 ∈ σ ∩ τ2 := Finset.mem_inter.mpr ⟨hxσ, x.2⟩
        simp [mt2emp] at hxinter
      have hnt : x.1 ∉ τ1 := by
        intro hxτ1
        have hxinter : x.1 ∈ τ1 ∩ τ2 := Finset.mem_inter.mpr ⟨hxτ1, x.2⟩
        simp [t1t2emp] at hxinter
      simp [w, c, hns, hnt]
