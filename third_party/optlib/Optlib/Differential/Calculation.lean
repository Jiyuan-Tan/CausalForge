...
theorem HasGradientAtFilter.comp
  ...
  -- `comp_hasFDerivAtFilter` now takes the product-filter tendsto (mathlib's
  -- `HasFDerivAtFilter` uses `Filter (F × F)`); build it from `hL` and `pure x`.
  exact hg.hasDerivAtFilter.comp_hasFDerivAtFilter hf (hL.prodMap (tendsto_pure_pure f x))
...
theorem HasGradientAtFilter.sum (h : ∀ i ∈ u, HasGradientAtFilter (A i) (A' i) x L) :
  ...
  -- `HasFDerivAtFilter.sum` yields the point-free `∑ i ∈ u, A i`; bridge to the
  -- pointwise `fun y => ∑ i ∈ u, A i y` in the statement.
  rw [show (fun y => ∑ i ∈ u, A i y) = ∑ i ∈ u, A i from
...
lemma equiv_lemma_mul : c x • (toDual 𝕜 F) d' + d x • (toDual 𝕜 F) c'
  ...
  -- `toDual` is conjugate-linear; unfold the additive/star-smul structure and use
  -- that `starRingEnd` is involutive.
  rw [map_add, map_smulₛₗ, map_smulₛₗ]; simp
