...
lemma bounded_lowersemicontinuous_to_epi_closed (f : E → ℝ) (hc : LowerSemicontinuousOn f univ)
  ...
  simpa using
    (lowerSemicontinuousOn_iff_isClosed_epigraph (f := f) (s := univ) isClosed_univ).mp hc
...
lemma continuous_positive_direction [NormedSpace ℝ E] (h : ContinuousAt f x) (hx : f x > 0) (v : E) :
  ...
  have : ‖v‖ > 0 := by simpa using (norm_pos_iff.mpr hv)
...
theorem HasGradient_iff_Convergence_Point {f'x : E}:
  ...
  constructor
  · intro h; apply HasGradient_Convergence (f' := fun _ => f'x)
    exact h
  · apply Convergence_HasGradient (f' := fun _ => f'x)
...
lemma gradient_norm_sq_eq_two_self (x : E) :
  ...
  . intro x' dles
    ...
    have hid : ‖x'‖ ^ 2 - ‖x‖ ^ 2 - inner ℝ ((2 : ℝ) • x) (x' - x) =
        ‖x' - x‖ ^ 2 := by
      rw [← real_inner_self_eq_norm_sq (x' - x)]
      simp only [inner_sub_left, inner_sub_right, real_inner_self_eq_norm_sq,
        real_inner_smul_left]
      rw [real_inner_comm x x']
      ring
    rw [hid, abs_of_nonneg (sq_nonneg _), pow_two]
    simpa [norm_sub_rev] using mul_le_mul_of_nonneg_right dles (norm_nonneg (x' - x))
...
lemma gradient_of_const_mul_norm (l : ℝ) (z : E) :
  ...
  have hscale : ((l / 2) • (2 : ℝ) • z) = l • z := by
    rw [smul_smul]
    congr 1
    ring
  simpa [h, smul_smul, mul_comm, mul_left_comm, mul_assoc, hscale] using
    (h1.const_smul' (c := l / 2))
  ...
  · intro x' dles; field_simp; rw [abs_div]; simp
    have eq1 (u v : E) (e : ℝ) (dle : ‖u - v‖ ≤ e) :
      ...
      have hid : ‖v‖ ^ 2 - ‖u‖ ^ 2 - inner ℝ ((2 : ℝ) • u) (v - u) =
          ‖v - u‖ ^ 2 := by
        rw [← real_inner_self_eq_norm_sq (v - u)]
        simp only [inner_sub_left, inner_sub_right, real_inner_self_eq_norm_sq,
          real_inner_smul_left]
        rw [real_inner_comm u v]
        ring
      rw [hid, abs_of_nonneg (sq_nonneg _), pow_two]
      simpa [norm_sub_rev] using mul_le_mul_of_nonneg_right dle (norm_nonneg (v - u))
    let u := s - x
    have hu : u = s - x := rfl
    let v := x' - x
    have hv : v = x' - x := rfl
    ...
    have hle : |‖v‖ ^ 2 - ‖u‖ ^ 2 - inner ℝ ((2 : ℝ) • u) (v - u)| ≤ e * ‖u - v‖ := by
      apply eq1
      rw [hu, hv]
      simpa [norm_sub_rev] using dles
    nlinarith [mul_nonneg (le_of_lt ep) (norm_nonneg (u - v))]
...
lemma expansion (hf : ∀ x : E, HasGradientAt f (f' x) x) (x p : E) :
  ...
  have e3 : ∀ t, inner ℝ (f' (x + t • p)) p = g' t := by
    intro t
    rfl
  rw [e1, e2]
...
-- drift: `WithLp 2 (E × F)` no longer exposes product fields via `.1`; use `WithLp.fst`.
lemma fst_norm_le_prod_L2 (z : WithLp 2 (E × F)) : ‖z.fst‖ ≤ ‖z‖ := by
  have h : ‖z.fst‖ ^ 2 ≤ ‖z‖ ^ 2 := by linarith [WithLp.prod_norm_sq_eq_of_L2 z, sq_nonneg ‖z.snd‖]
  apply nonneg_le_nonneg_of_sq_le_sq (norm_nonneg _)
  rwa [← pow_two, ← pow_two]

-- drift: `WithLp 2 (E × F)` no longer exposes product fields via `.2`; use `WithLp.snd`.
lemma snd_norm_le_prod_L2 (z : WithLp 2 (E × F)) : ‖z.snd‖ ≤ ‖z‖ := by
  have h : ‖z.snd‖ ^ 2 ≤ ‖z‖ ^ 2 := by linarith [WithLp.prod_norm_sq_eq_of_L2 z, sq_nonneg ‖z.fst‖]
  apply nonneg_le_nonneg_of_sq_le_sq (norm_nonneg _)
  rwa [← pow_two, ← pow_two]

-- drift: `WithLp 2 (E × F)` product projections are now `fst`/`snd`.
...
lemma norm_prod_right_zero (x : E) :
    -- drift: construct `WithLp` product values explicitly with `WithLp.toLp`.
    @norm (WithLp 2 (E × F)) _ (WithLp.toLp 2 (x, (0 : F))) = ‖x‖ := by
  rw [WithLp.prod_norm_eq_of_L2] ; simp

lemma norm_prod_left_zero (y : F):
    -- drift: construct `WithLp` product values explicitly with `WithLp.toLp`.
    @norm (WithLp 2 (E × F)) _ (WithLp.toLp 2 ((0 : E), y)) = ‖y‖ := by
  rw [WithLp.prod_norm_eq_of_L2] ; simp
...
instance instIsBoundedLinearMapL2equiv :
  ...
  bound := by
    ...
    · intro z
      rw [Prod.norm_def]
      change max ‖z.fst‖ ‖z.snd‖ ≤ 2 * ‖z‖
      exact max_le (by nlinarith [fst_norm_le_prod_L2 z, norm_nonneg z])
        (by nlinarith [snd_norm_le_prod_L2 z, norm_nonneg z])
