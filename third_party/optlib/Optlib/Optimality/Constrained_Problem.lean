...
lemma posTangentCone_localmin_inner_pos {f : E → ℝ} {loc : E} (hl : IsLocalMinOn f p.FeasSet loc)
  ...
  intro v hv
  simpa using hl.hasFDerivWithinAt_nonneg hf.hasGradientAt.hasFDerivAt.hasFDerivWithinAt hv
...
lemma LICQ_Axfullrank (x : EuclideanSpace ℝ (Fin n)) (LIx : p.LICQ x)
    ...
    -- drift: EuclideanSpace rows are now `WithLp`; matrix rows need `.ofLp`.
    (eq : M = fun i : (p.active_set x) ↦
        (if i.1 ∈ τ then gradient (p.equality_constraints i) x
  ...
  · simp
    ...
    simp
    rw [eq]
    simpa [Function.comp_def, Matrix.row, WithLp.coe_linearEquiv] using
      (LIx.map' ((WithLp.linearEquiv 2 ℝ (Fin n → ℝ)).toLinearMap) (by simp))
...
lemma LICQ_injM (z : EuclideanSpace ℝ (Fin n)) (m : ℕ)
  ...
  have hcard : Fintype.card ((p.active_set x) ⊕ (Fin (n - m))) = Fintype.card (Fin n) := by
    simp
    rw [aux]
  have hBt : ∀ (v : (p.active_set x) ⊕ (Fin (n - m)) → ℝ), Bᵀ *ᵥ v = 0 → v = 0 := by
    intro v Btveq0
    let y := v ∘ Sum.inl
    let z := v ∘ Sum.inr
  ...
  simpa using ((inj_transpose_iff_inj_of_sq hcard).1 hBt z Bzeq0)
...
lemma LICQ_strictfderiv_Ax_elem {x : EuclideanSpace ℝ (Fin n)}
  ...
  rw [eventually_iff, Metric.mem_nhds_iff] at h; rcases h with ⟨ε, εpos, hε⟩
  intro i; by_cases hi : i.1 ∈ τ
  · rw [ceq, Jzeq, Aeq]; simp [hi]
    apply ContDiffAt.hasStrictFDerivAt' (conte i hi)
  ...
  · have hi' : i.1 ∈ σ := by
      ...
      exact hi2.1
    rw [ceq, Jzeq, Aeq]; simp [hi]
    apply ContDiffAt.hasStrictFDerivAt' (conti i hi')
...
lemma eq_lemma {y z : EuclideanSpace ℝ (Fin n)} {n : ℕ} (h : ‖(n : ℝ) • y‖ ≠ 0) :
  ...
  have hnpos : (0 : ℝ) < n := by
    exact_mod_cast Nat.pos_of_ne_zero h.1
  have hnorm : ‖(n : ℝ) • y‖ = (n : ℝ) * ‖y‖ := by
    simp [norm_smul, Real.norm_eq_abs, abs_of_pos hnpos]
  have hynorm : ‖y‖ ≠ 0 := by
    simpa using h.2
  ext i
  simp [hnorm, sub_eq_add_neg, smul_add, smul_smul]
  left
  field_simp [h.1]
...
lemma comap1 {x : EuclideanSpace ℝ (Fin n)} {m : ℕ}
  ...
  · intro z zin; simp at zin
    ...
    have zin' : z ∈ Metric.ball 0 a := by
      simp; calc
        ‖z‖ ≤ c * ‖Mx z‖ := antil
        _ < c * (a / c) := mul_lt_mul_of_pos_left (by linarith [zin]) hc'
        _ = a := by field_simp
    exact ha zin'
...
lemma comap2 (hv : v ≠ 0):
  ...
  have denom_pos : 0 < a + ‖v‖ := add_pos apos vpos
  have denom_ne : a + ‖v‖ ≠ 0 := ne_of_gt denom_pos
  ...
  · intro z zin; simp at zin;
    ...
    simp [ze] at zin; rw [norm_smul] at zin
    have : 0 < ‖z‖ := by refine lt_of_le_of_ne (norm_nonneg z) ?_; symm; simp [ze]
    rw [Real.norm_eq_abs, abs_of_pos (inv_pos.mpr this), inv_mul_eq_div] at zin
...
lemma LICQ_tendsto {x : EuclideanSpace ℝ (Fin n)} {m N : ℕ}
  ...
  have eq5 : deriv' =ᶠ[atTop] φ ∘ (fun i : ℕ ↦ (i : ℝ) • (d i - x)) := by
    ...
    simp only [deriv', φ, Function.comp_apply]
    rw [if_neg neq]
    simpa [one_div] using (@eq_lemma n (d i - x) v i neq)
...
theorem LICQ_linearized_feasible_directions_sub_posTangentCone
  ...
  let Ax : Matrix (p.active_set x) (Fin n) ℝ := fun i ↦ (gradc x i).ofLp -- Jacobi at x
  let m := (p.active_set x).card
  have mlen : m ≤ n := LICQ_mlen x LIx (by rfl)
  have existZ : ∃ (Z : Matrix (Fin n) (Fin (n - m)) ℝ), Ax * Z = 0 ∧ Matrix.rank Z = (n - m) := by
    apply LICQ_existZ x LIx <;> rfl
  rw [LICQ] at LIx;
  ...
  let A : EuclideanSpace ℝ (Fin n) → Matrix (p.active_set x) (Fin n) ℝ :=
    fun z ↦ (fun i ↦ (gradc z i).ofLp) -- compose the gradient matrix
  ...
  have Rzgrad : HasStrictFDerivAt Rz Mx x := by
    ...
    · let N : EuclideanSpace ℝ (Fin n) →L[ℝ] (Fin (n - m) → ℝ) :=
      ...
      show HasStrictFDerivAt (fun y : EuclideanSpace ℝ (Fin n) ↦ N (y - x)) N x
  ...
  have dtend_metric := dtend
  rw [tendsto_iff_norm_sub_tendsto_zero, NormedAddCommGroup.tendsto_nhds_zero] at dtend_metric; simp at dtend_metric
  obtain ⟨ε, εpos, inactive⟩ := LICQ_inactive_nhds x xf conti
  obtain ⟨N', dtendx⟩ := dtend_metric ε εpos

  refine mem_tangentConeAt_of_seq (R := NNReal) atTop (fun n : ℕ ↦ (n : NNReal))
    (fun n ↦ d n - x) dtend0 ?_ ?_
  · rw [eventually_atTop]
    use max N N'; intro nn hnn
    have hdnn : d nn ∈ p.FeasSet := by
      ...
      have hv1fun : c (d nn) = (nn : ℝ)⁻¹ • Ax *ᵥ v := by
        change c (d nn) = WithLp.ofLp ((nn : ℝ)⁻¹ • (toEuclideanLin Ax) v)
        exact congrArg WithLp.ofLp hv1
    ...
    simpa [sub_eq_add_neg, add_comm, add_left_comm, add_assoc] using hdnn
...
lemma subtype_sum (σ τ : Finset ℕ) (f : σ → EuclideanSpace ℝ (Fin n))
  ...
  have eq1 : ∑ i : σ, f i = ∑ i ∈ σ, f₁ i := by
    simpa [f₁] using (Finset.sum_attach σ f₁)
  have eq2 : ∑ i : {x // x ∈ σ ∩ τ},
...
theorem first_order_neccessary_general (p1 : Constrained_OptimizationProblem (EuclideanSpace ℝ (Fin n)) τ σ)
  ...
  · obtain h1 := local_Minimum_constraint_qualification_descent_direction'' loc hl hf
    ...
    · unfold Lagrange_function
      ...
      simpa using (DifferentiableAt.fun_sum (u := Finset.univ)
        (A := fun i : τ => fun m => lam i * p1.equality_constraints (↑i) m)
        (fun i _ => (he1 i i.2).const_mul (lam i)))
      apply DifferentiableAt.sub hf.differentiableAt
      simpa using (DifferentiableAt.fun_sum (u := Finset.univ)
        (A := fun i : τ => fun m => lam i * p1.equality_constraints (↑i) m)
        (fun i _ => (he1 i i.2).const_mul (lam i)))
      simpa using (DifferentiableAt.fun_sum (u := Finset.univ)
        (A := fun i : σ => fun m => mu1 i * p1.inequality_constraints (↑i) m)
        (fun i _ => (hc1 i i.2).const_mul (mu1 i)))
    constructor
    · intro j; simp [mu1]
...
theorem Linear_linearized_feasible_directions_eq_posTangentCone
  ...
  let c : ℕ → NNReal := fun k ↦ ⟨(k + (1 : ℝ)) / t_, by positivity⟩
  refine mem_tangentConeAt_of_seq (R := NNReal) atTop c z ?_ ?_ ?_
  · have hscalar : Filter.Tendsto (fun k : ℕ ↦ t_ / ((k : ℝ) + 1)) atTop (𝓝 0) := by
      exact tendsto_const_nhds.div_atTop
        (tendsto_atTop_mono
          (fun k : ℕ => le_add_of_nonneg_right (zero_le_one' ℝ))
          tendsto_natCast_atTop_atTop)
    simpa [z] using hscalar.smul (tendsto_const_nhds (x := v))
  ...
  · apply tendsto_atTop_of_eventually_const (i₀ := 1)
    intro i hi; simp [c, z]
    change (((i : ℝ) + 1) / t_) • ((t_ / ((i : ℝ) + 1)) • v) = v
    rw [smul_smul]
    field_simp [ne_of_gt ht_]
    simp
