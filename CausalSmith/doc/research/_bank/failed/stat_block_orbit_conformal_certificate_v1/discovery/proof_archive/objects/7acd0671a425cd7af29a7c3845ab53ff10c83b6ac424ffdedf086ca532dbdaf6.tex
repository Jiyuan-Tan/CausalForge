...
\tag{1}
\]
Indeed, the \(k_a\)-th order statistic is below the target score exactly when at least \(k_a\) calibration scores are strictly below it. Thus the rank event, including all ties, depends only on the induced total preorder. If \(k_a=M+1\), the convention \(Q_{k_a}=+\infty\) makes the event identically true, so it imposes no score information.

Conversely, let a total preorder on \(\mathcal O_a\) have ordered nonempty tie classes \(C_{a,1}\prec_a\cdots\prec_a C_{a,d_a}\). If \(d_a=1\), assign score zero to its only class. If \(d_a>1\), assign \((j-1)K/(d_a-1)\) to every orbit in \(C_{a,j}\). Since \(K>0\), this is a map into \([0,K]\) inducing exactly the given preorder, with \(d_a\le m_a\) levels. The number of total preorders on an \(m_a\)-element labelled set is
\[
F_{m_a}=\sum_{d=1}^{m_a}d!\left\{\begin{matrix}m_a\\ d\end{matrix}\right\},
\tag{2}
\]
where the braces are Stirling numbers of the second kind. The objective in \(\texttt{def:coverage-frontier}\) is therefore constant on each of the finitely many \(F_{m_0}F_{m_1}\) preorder pairs, and every pair has an admissible representative. This proves attainment and exactness over the declared class of bounded orbit-constant score maps. It does not enlarge that class to all laws in \(\mathcal P_{\mathcal G,K}\).
...
\tag{3}
\]
The product in \(\texttt{ass:selector-auxiliary-randomization}\) gives conditional independence of the two auxiliary selector draws given their shared assignment, while \(\texttt{ass:known-matched-randomization}\) integrates that assignment in (3). Across replicates the calibration orbit pairs are independent and identically distributed with mass function \(R_w\). Thus (3) retains all within-replicate cross-arm dependence induced by assignment; it is not replaced by the product of its marginals.

Fix a target member \(v\) and a pair of total preorders, let \(O_a(v)\) be the orbit containing \((v,a)\), and set
\[
...
\tag{4}
\]
When both \(k_0,k_1\le M\), initialize \(d_0(0,0)=1\), initialize every other entry at zero, and use
\[
...
\tag{6}
\]
Together with (1), (6) is exactly the conditional joint rank-event probability for this target and preorder pair. Since \(T\sim q_{\mathrm{tar}}\) is the one common target, the probability in the frontier objective is the coherent finite sum
\[
\sum_{v\in\mathcal V}q_{\mathrm{tar}}(v)\sum_{r=0}^{k_0-1}\sum_{s=0}^{k_1-1}d_M^{(v)}(r,s).
\tag{7}
\]
The single sum over \(v\) preserves the coherence of the two target-arm lifts, while the full vector in (4) preserves calibration-arm coupling.

For each target and preorder pair, forming (4) scans the \(m_0m_1\) entries of \(R_w\), and (5) performs four constant-time updates per retained state at each of \(M\) stages. Rolling the current and next arrays gives
\[
O(m_0m_1+Mk_0k_1)\quad\text{arithmetic operations and}\quad O(k_0k_1)\quad\text{working memory}.
\tag{8}
\]
Consequently, after \(R_w\) is tabulated, full enumeration over all targets and preorder pairs uses
\[
O\!\left(F_{m_0}F_{m_1}|\mathcal V|\{m_0m_1+Mk_0k_1\}\right)
\tag{9}
\]
arithmetic operations and \(O(k_0k_1)\) working memory. If exactly one rank equals \(M+1\), let \(k_c\le M\) denote the other rank. The boundary-arm event is certain. Summing (4) over its indicator coordinate and applying the one-dimensional part of \(\texttt{lem:four-cell-rank-event-dynamic-program}\) gives the full-enumeration cost
\[
O\!\left(F_{m_0}F_{m_1}|\mathcal V|\{m_0m_1+Mk_c\}\right)
\quad\text{and memory}\quad O(k_c).
\tag{10}
\]
If both ranks equal \(M+1\), both events are certain, so the objective is returned as zero in constant time without enumerating targets or preorders. This proves all asserted computational bounds and exact boundary reductions.
...
\tag{11}
\]
and then selects that block's unique observed arm-\(a\) member. This kernel is observable and is equivariant under all pair swaps. Put
\[
...
\tag{16}
\]
This table exhausts all \(16^2\) binary pairs. Its largest entry is \(208582976/320000000=3259109/5000000\). Therefore the largest binary frontier objective is
\[
\frac{3259109}{5000000}-\frac12=\frac{759109}{5000000},
\]
and its gap below (14) is
\[
