...
\tag{2}
\]
It preserves the unchanged \(R_w\), coherent \(P_T^C\), local weights, common selector, common target, and exact memo key. The vertex, edge, zero-corner face, and rank-one prepasses followed by the least-charge residual dispatcher give exactly the displayed seven-way partition. In particular, the face and rank-one strata contribute respectively
\(\sum_{\pi\in\Pi_{\mathrm{face}}}\mathsf F_M(\pi)\) and \(\sum_{\pi\in\Pi_{\mathrm{rank1}}}\mathsf R_{M,k_0,k_1}\) to \(\mathsf B\), contribute no nonconstant term to \(\mathsf W\), and do not trigger the multinomial factorial table. The cited evaluator lemma also proves both time formulas and the memory formula. Thus every two-finite-rank computational assertion follows.
...
For the rank-one strictness claim, the coherent evaluator constructs the legal two-block key \(\pi=(1/4,1/4,1/4,1/4)\) and verifies the ranks \(k_0=n\), \(k_1=n+2\) from the displayed admissible levels. Its determinant is zero, so the product-binomial identity gives the asserted value. The same audit proves \(\Theta(M)\) exact time and constant workspace for the new branch, compared with \(\Theta(M^2)\) time and \(\Theta(M)\) workspace for the least-charge former branch. This computational refinement evaluates the same probability in equation (2), so it leaves every statistical, target, selector, boundary, pair, empty-library, and weighting assertion unchanged.