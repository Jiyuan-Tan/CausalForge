Fix the calibrated value of \(\kappa\), fix \(\gamma>d\), and put \(q_0=\min\{\beta,\gamma\}\). We first derive the global smoothness that the estimator will use. At \(t=t_0=c\), \(\mathrm{ass:lower\text{-}margin}\) and calibration give
\[
...
We now give one estimator that uses only the sample, the known cube, and the frozen constants \(d\), \(q_0\), and \(c_\pi\). Let \(p_0=\lceil q_0\rceil-1\), use \(W_{x,h}\), \(K_{x,h}\), and the shifted-Legendre vector \(r(u)=r_{x,h,p_0}(u)\) from \(\mathrm{def:window\text{-}basis}\), and write \(r_x=r(x)\). For \(a\in\{0,1\}\), define
\[
...
\]
By \(\mathrm{lem:uniform\text{-}design}\), the change of variables \(z=T_{x,h}(u)\), orthonormality of the shifted-Legendre basis, and strict overlap, for every vector \(v\),
\[
2^{-d}c_\pi\|v\|_2^2
\le v^\top Q_a(x)v
...
\]
The final truncated shell \(2^JA\varepsilon_n<\Delta(X)\le c\) has \(2^JA\varepsilon_n\ge c/2\); its contribution is therefore at most \(Ce^{-c''/\varepsilon_n}\le C\varepsilon_n^2\). Since \(\sum_{j\ge0}4^je^{-c'2^j}<\infty\), Tonelli's theorem and \(\mathrm{def:weighted\text{-}loss}\) now give
\[
...
=Cn^{-2q_0/(2q_0+d)}.
\]
This is a risk bound for the single data-driven estimator just constructed, not for a law-specific oracle. Taking the infimum over all measurable set estimators in \(\mathrm{def:minimax\text{-}risk}\) and substituting \(q_0=\min\{\beta,\gamma\}\) proves both conclusions.