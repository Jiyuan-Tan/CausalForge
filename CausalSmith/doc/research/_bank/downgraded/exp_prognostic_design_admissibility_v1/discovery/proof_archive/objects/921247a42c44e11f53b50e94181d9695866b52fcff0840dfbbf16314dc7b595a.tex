...
\tag{1}
\]
The homogeneous even-block result in the partition frontier lemma, specialized to \(B=k\) blocks of size \(m=2\), has \(p=k-1\), \(\delta_m=2\), and
\[
\beta=\frac{k-1}{2k-1}
...
\tag{7}
\]
Thus \((5)\)--\((7)\) are an attaining design and matching converse over the full class.  The balanced-covariance realizability lemma converts the covariance mixture into a mean-zero exactly balanced assignment law.  Rescaling \((6)\) by \(4M/n\) yields
\[
R^*(n,S_{\mathrm{pair}})
=\frac{2M(n-2)}{(n-1)^2}.
\tag{8}
...
It remains only to verify the exact four-unit weights.  When \(n=4\), \(\alpha_4=1/3\), and, up to sign, the balanced assignments form three sign-pairs.  Complete randomization assigns weight \(1/3\) to each sign-pair, while the within-pair law in the four-unit construction assigns weights \((0,1/2,1/2)\), with zero on the omitted sign-pair.  Their mixture with complete-randomization weight \(1/3\) therefore has weights
\[
\left(
\frac13\frac13,
