...
\tag{5}
\]
Taking \(A=c_nP_H\) gives \(\gamma(S)\leq1\).  Hence the improving squared-quality set is \((\gamma(S),1]\) when \(\gamma(S)<1\), and is empty when \(\gamma(S)=1\).  Equations \((4)\)--\((5)\) prove the three-way equivalence and the formula for \(\rho^*(S)\).
