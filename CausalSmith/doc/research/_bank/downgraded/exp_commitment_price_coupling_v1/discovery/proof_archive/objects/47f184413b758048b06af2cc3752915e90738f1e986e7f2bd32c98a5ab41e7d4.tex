...
\]
Then \(\alpha_s\ge0\), \(\sum_s\alpha_s=1\), and the identity \((-s)(-s)^\top=ss^\top\) gives
\[
...
Fix \(j\).  By the spectral decomposition of \(G_j\), every \(x\in\operatorname{range}(G_j)\) has a representation \(x=G_j^{1/2}u\) with \(u\in\operatorname{range}(G_j)\), and the unique such \(u\) satisfies
\[
...
\tag{3}
\]
The last equality follows from \((1)\) and the definition of \(A_{js}\).  It is exact for noncommuting matrices.

Under \(\texttt{ass:fixed-potential-outcomes}\), \(\texttt{ass:no-interference}\), and \(\texttt{ass:assignment-only-randomness}\), the algebra in \(\texttt{lem:variance-and-spectral-risk}\) gives
\[
...
For the reverse inequality, take all \(\alpha_s>0\) and choose \(t\) sufficiently negative.  Because every \(\delta_j>0\), this makes every \(F_j(\alpha,t)\) positive definite.  Hence the finite-dimensional conic program has a relative-interior feasible point.  Separating the open set of strict improvements from its closed convex feasible image gives nonnegative separating matrices \(X_j\); the coefficient of the free coordinate \(t\) normalizes them exactly as in \((11)\), and minimizing the affine coefficient over the simplex produces the minimum over \(s\) in \((12)\).  Thus the separating bound is attained and equals the primal value, which proves \((\mathrm C)\).  Equivalently, this is the direct strict-feasibility proof of finite-dimensional semidefinite strong duality for the displayed program.  At equality, the same calculation yields the checkable complementary conditions
\[
