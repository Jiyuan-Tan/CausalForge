\textbf{Proof.}
For arm \(a\), write \(F_{a,m_a}\) for the distribution function of \(\widehat F_a\).  We first prove the required empirical-law convergence without assuming a limit theorem.  For fixed rational \(t\in[0,1]\), the variables
\[
...
Fix an integer \(K\ge1\) and the grid \(x_j=j/K\).  Monotonicity of the two distribution functions implies, for \(y\in[x_j,x_{j+1}]\),
\[
 |F_{a,m_a}(y)-F_a(y)|
 \le
...
\tag{4}
\]
After integration and summation over \(j\), (4) gives
\[
 \limsup_{m_a\to\infty}
...
Every block of \(\widehat Q_e-Q(c_e)\) is a convergent scalar multiplying either \(I_n\) or \(\mathbf 1\mathbf 1^{\top}\).  Since \(n\) is fixed, \(\lVert I_n\rVert_{\mathrm{op}}=1\) and \(\lVert\mathbf 1\mathbf 1^{\top}\rVert_{\mathrm{op}}=n\), so \(M_m\to0\) almost surely. \(\square\)