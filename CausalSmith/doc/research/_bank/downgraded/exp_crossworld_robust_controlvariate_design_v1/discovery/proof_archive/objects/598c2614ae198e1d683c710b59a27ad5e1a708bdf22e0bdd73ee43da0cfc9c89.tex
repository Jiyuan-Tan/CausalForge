...
\le w^+(B).                                                        \tag{4}
\]
Thus \(V_Z\le w^+(B)\) for every published pseudoinverse class.  Taking the infimum in (3)--(4) gives \(V_Z\le V_+\), which together with (2) proves
\[
...
\textbf{Sanity check.}
If an ordinary minimizer exists, its image satisfies the equality defining \(\mathcal A_{\mathrm{mm}}^+\) by (5).  If no ordinary minimizer exists, the infimum equality (5) remains valid, while the full-class equality set may consistently be empty.