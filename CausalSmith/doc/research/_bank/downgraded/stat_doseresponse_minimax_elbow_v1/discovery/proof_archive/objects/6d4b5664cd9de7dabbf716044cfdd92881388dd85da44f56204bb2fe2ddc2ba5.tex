...
\]
By the path typing, \(Y(A)=e(\mathbf Y,A)\) almost surely, and consistency gives \(Y=Y(A)\) almost surely. Hence
\[
...
\]
This finite space is standard Borel, and \(e\) is jointly measurable. In causal completion zero take \(\mathbf Y=y_0\) almost surely; in causal completion one take \(\mathbf Y=y_1\) almost surely. In both completions \(\mathbf Y\) is deterministic and therefore conditionally independent of \(A\) given \(X\). Moreover, \(P(A=t_0)=0\), so
\[
e(\mathbf Y,A)=0=Y
\quad\text{almost surely}
\]
in both completions, proving consistency. The observed law is identical, but
\[
\mathbb E[Y^{(0)}(t_0)]=0,
\qquad
