Throughout the proof, the phrase ``on the good-Gram event'' means multiplication by
\[
\mathbf1_{G_h},
\qquad
G_h=\{\lambda_{\min}(\widehat D_h)\ge\lambda_*\},
\]
which is the truncation used by the estimator in Definition~\texttt{def:beta-free-handle}. Condition on \(I_\mu\) and \(I_{\mathrm{aux}}\). Put \(N=|I_{\mathrm{lp}}|\), \(\bar f=P_X\widehat\pi_0\), \(\bar m=P_X\widehat\mu_0\), and
\[
\bar\xi_i=
\frac{\bar f}{\widehat\pi_0(X_i)}\{Y_i-\widehat\mu_0(X_i)\}+\bar m.
\]
The construction and the separate value bounds give \(|\widehat\mu_0|\le M_Y\) and \(c/2\le\widehat\pi_0\le2/c\), so \(|\bar\xi_i|\le C(M_Y,c)\). Define
\[
D_h=\mathbb E_P[K_h(A)S_h(A)S_h(A)^\top],
\qquad
\gamma_h=D_h^{-1}\mathbb E_P[K_h(A)S_h(A)\bar\xi].
\]
The population-Gram conclusion of Lemma~\texttt{lem:design-adaptive-local-polynomial-bias} gives the inverse bound. Direct integration of the two-sided positivity bounds in Definition~\texttt{def:holder-class} gives the required marginal-density bounds. Hence \(\|D_h^{-1}\|_{\mathrm{op}}+\|\gamma_h\|\le C\). On \(G_h\), exact normal-equation algebra gives
\[
