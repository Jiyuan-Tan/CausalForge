...
\[
 ...
 \end{aligned} \tag{7}
\]
Thus \(\mu_{d,1}-\mu_{d,0}=\tau\) for both designs throughout this local identified submodel.  In particular, at \(P_\epsilon\), \(Y(1)=Y(0)=U\), so \(\tau=0\), and symmetry also gives \(\mu_{d,a}=\mathbb E[W_y/y\mid A=a,G=E]=0\).  By \(\texttt{lem:ikmw-fixed-system-efficiency}\), bridge uniqueness and (1) make the source gradients canonical for this causal target on the constrained local submodel.  Hence the profiled values compared below are causal efficiency bounds there, not merely variances of unrelated bridge functionals.
