...
\[
 ...
 =\mathbb E[U\mid G=O]. \tag{7}
\]
The right side does not depend on \(a\), while \(Y(1)=Y(0)=U\); hence \(\mu_{d,1}-\mu_{d,0}=0=\tau\).  We have now verified Assumptions 1--4, unique latent outcome and selection bridges, bijectivity, and finite score moments before invoking the cited IKM efficiency theorem.  Lemma~\texttt{lem:ikmw-fixed-system-efficiency} therefore makes the source gradient canonical for \(\tau\) in the fixed-system local model at the center law.
...
\]
Strict cell positivity makes \(\ell_{a,Q}\) finite.  Equations (19)--(20) are the latent IKM bridge equations by construction.  Conditional independence of the proxy coordinates given \(U\), followed by iterated expectation, makes them solve the two observed bridge equations.  The observed matrix of \(T_d\) is a product of the two invertible proxy channels and positive diagonal reweighting matrices; it is therefore invertible.  Thus the observed bridges are unique.  This is the required nearby-law finite-dimensional inversion, rather than reuse of the center formulas.

The coefficients in (19)--(20) are rational continuous functions of the cell probabilities.  Prediction risks are finite-cell conditional-variance functionals and are continuous while cells remain positive.  Source-score second moments, and hence profiled values, are continuous functions of the cell probabilities and bridge coefficients.  At \(P_\epsilon\), (1)--(2), (10), (11)--(16) give strictly positive regularity, variance, predictive-selection, and design-value-selection margins.  Continuity therefore furnishes a radius \(r_\epsilon>0\) on which regularity and the same two strict optimizer assignments persist.  The denominator of \(\rho_{\mathrm{pred}}\) is positive by (14)--(15), so the ratio is continuous as well.  Shrinking \(r_\epsilon\) if necessary gives
\[
