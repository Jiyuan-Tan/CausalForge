Fix the prescribed \(\chi>1\), catalogue, costs, cap, and two role assignments exactly as in Definition \(\mathrm{def{:}binary\text{-}witness}\). The \emph{binary constrained finite-cell model} \(\mathcal M_{\mathrm{bin}}(\chi)\) is the family of laws \(P\) with constant \(X\) and constant \(S_{2d}\), latent variables \(U,T,W_1,W_2,W_3,W_4\in\{-1,1\}\), \(A=(T+1)/2\), and potential variables
\[
