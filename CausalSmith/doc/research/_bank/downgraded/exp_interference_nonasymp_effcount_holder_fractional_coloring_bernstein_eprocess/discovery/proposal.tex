...
%  Locked parameters: qid=exp\_interference\_nonasymp\_effcount, specialization=holder\_fractional\_coloring\_bernstein\_eprocess
%
...
%    regression)` precisely. Only for a legacy numbered qid (`q1_*`, etc.),
%    use the retired Q1 pool-mode and
...
\title{causalsmith Stage -1 proposal: \texttt{exp\_interference\_nonasymp\_effcount} / \texttt{holder\_fractional\_coloring\_bernstein\_eprocess}%
...
%   assignment, target, regression) precisely for qid=exp\_interference\_nonasymp\_effcount
%   specialization=holder\_fractional\_coloring\_bernstein\_eprocess. Only for a legacy numbered qid (`q1_*`, etc.),
%   use the retired Q1 pool-mode and
%   reproduce that block VERBATIM from
%   CausalSmith/doc/panel_formula_question_generator_note_with_common_prompts.tex
...
%   - Cite relevant in-catalogue causalsmith proposals (by qid + headline statement) and `doc/API.md`
