...
\]
The posterior mean is the squared-error Bayes action, proving posterior risk one in both orbit types. Moreover, any other binary assignment on the single clique reveals neither target sign, hence has posterior variance two; fixing all non-target outcomes across the four atoms prevents them from conveying additional information. Thus every design-estimator pair has Bayes risk at least one. Together with the displayed all-control upper procedure, this also verifies the unrestricted calibration \(R_2(G_{1,s})=1\).