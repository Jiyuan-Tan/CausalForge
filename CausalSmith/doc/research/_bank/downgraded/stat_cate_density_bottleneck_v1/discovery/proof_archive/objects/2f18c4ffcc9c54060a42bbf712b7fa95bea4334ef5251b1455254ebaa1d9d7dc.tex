Fix the class constants and a law \(P\in\mathcal P_{\mathrm{band},n}\).  All constants below may depend on those fixed constants, on \(d,\alpha,\beta,\gamma\), and on \(x_0\), but not on \(P\) or \(n\).
...
\]
Consequently the weighted conditional normal equation has the unique regression target \(\tau_P(X)\).  This establishes the conditional-target assertion, and \(\mathrm{lem:cate\mbox{-}identification}\) supplies its causal interpretation.
...
\end{aligned}
\]
Consistency and bounded potential outcomes give \(|Y|\leq1\).  Also \(|\eta|\leq1\), and \(|p_h|\leq2+Ch^\gamma\) on \(\mathcal C\).  The boundedness of \(b\) and the density upper bound therefore yield
\[
...
Let \(\widehat\theta\) be the safeguarded oracle coefficient for this fold.  On \(G\), the safeguard is inactive, so the two displayed oracle normal equations give the exact identity
\[
\widehat\theta-\theta_h
...
\]
Finally, \(h=h_n=n^{-1/(2\gamma+d)}\) implies
\[
h^\gamma=(nh^d)^{-1/2}=n^{-\gamma/(2\gamma+d)},
\qquad
(nh^d)^{-1}=n^{-2\gamma/(2\gamma+d)}.
\]
For \(n<n_0\), both the retained truncation and \(\tau_P(x_0)\in[-2,2]\) bound the error by four, so enlargement of the constant covers these finitely many indices.  We conclude uniformly over \(P\in\mathcal P_{\mathrm{band},n}\) that
\[
\mathbb E_P\left|
\widehat\tau_n^{\mathrm{or}}(x_0)-\tau_P(x_0)
\right|
