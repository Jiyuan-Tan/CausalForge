...
For completeness, Lemma~\(\mathrm{lem:zeng\text{-}binary\text{-}collision\text{-}upper}\), followed by the elementary identity \(E[(Z-\theta)^2]=\operatorname{Var}(Z)+\{E(Z)-\theta\}^2\), gives the binary collision benchmark with heterogeneity remainder of order \(h\) in the dimension range considered here; the exponentially small bias term is absorbed by \(n^{-1}+d/n^2\).  Since
\[
 \min\{1,u,h\}\le h,
\tag{8}
\]
the selector's heterogeneity-dependent remainder is never larger in order.  If \(d=o(n\log(en))\), then \(u=o(1)\).  If in addition \(u=o(h)\), eventually \(u<\min\{1,h\}\), and the selected remainder is \(u=o(h)\).  This proves the strict-order comparison.  The construction uses known \(M\) and \(\sigma\); no inference or unknown-parameter adaptation statement is made.