...
and \(\varphi_k:=p_k(\nu_{1k}-\nu_{0k})\).  Then
\[
 \tau(P)/M=\sum_{k=1}^d\varphi_k,
 \quad z_{ak}=s_{ak}\nu_{ak},
 \quad |\nu_{ak}|\le1/2,
 \quad s_{ak}\ge\epsilon p_k.
\tag{1}
\]
The last two relations follow from mean normalization and overlap.  The conditional second-moment assumption gives
\[
...
\]
The balanced split and \(b_0=16\lambda_0\) give \(B_+\le B/4\) for all sufficiently large \(n\).  Binomial Chernoff inequalities and a union bound yield
\[
...
\[
 K\ge2,\qquad4(K+2)^2\le m_1,
 \qquad4(K+2)/m_1\le3B/4.
\tag{8}
\]
Equations~\((5)\)--\((6)\) and the covariance lemma give
\[
\begin{split}
...
\[
 ...
 \qquad
 n^\kappa\frac{d^2L^4}{n^3}\le C\frac{d^2}{n^2L^2},
\tag{10}
\]
...
For the heavy cells, conditional on the pilot and on the category and treatment indicators in \(I_1\), equation~\((2)\) gives
\[
...
\[
 ...
 \qquad
 R_a:=\frac1{m_1}\sum_{k\in\widehat{\mathcal H}}N_k\nu_{ak}\mathbf1\{N_{ak}=0\}.
\tag{13}
\]
...
\]
with value zero when too few distinct indices exist.  Hence the estimator is measurable and total, takes \(O(dK^2)\) arithmetic operations, and uses no outcome moment beyond the conditional second moment in \((2)\).