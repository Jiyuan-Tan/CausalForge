The following reduction is proved from the frozen core.  Put
\[
 L:=\log(en),\qquad b:=\frac1n+\frac d{n^2},\qquad
 u:=\frac{d^2}{n^2L^2},\qquad m:=\min\{u,\sigma^2\}.
\]
In the residual wedge, \(d/n^2<u<1\).  The elementary minimum comparison already established in Theorem~\(\mathrm{thm:fixed\text{-}interior\text{-}tightness\text{-}and\text{-}shrinking\text{-}radius\text{-}gap}\) gives
\[
 \frac12\{b+m\}
...
\tag{3}
\]
For the usual reflected choice \(\pi_-=1-\pi_+\), one has \(v_-=v_+\), so \((3)\) directly reveals every nonzero shift of \(t_h\).  Symmetric reflection therefore cannot satisfy even the required degree-two moment matching.  An asymmetric pair can cancel this one moment only by coupling its deviation and propensities; all higher mixed moments remain to be matched jointly.
...
\tag{6}
\]
on the light range.  Assumption~\(\mathrm{ass:overlap}\) implies that \(s_{1k}/s_{0k}\) and \(s_{0k}/s_{1k}\) are bounded by constants depending only on \(\epsilon\).  Equations~\((4)\)--\((6)\), together with Assumption~\(\mathrm{ass:approximate\text{-}homogeneity}\), therefore give
\[
 \left|t+\sum_kH_k(t)\{r_K(s_{0k})+r_K(s_{1k})\}-\vartheta\right|
 \le C_{\epsilon}\{\sigma+|t-\vartheta|\}\frac{dB}{K^2}.
\tag{7}
\]
Taking \(B\asymp L/n\) and \(K\asymp L\) makes the square of the right side in \((7)\) at most
\[
 C_{\epsilon}u\{\sigma^2+(t-\vartheta)^2\}.
\tag{8}
\]

This also proves a complete conditional reduction of the upper route.  If a total clipped heavy--light factorial lift \(\widehat S_K(t)\) of \(S_K(t)\) were shown to satisfy
\[
 E\!\left[(\widehat S_K(t)-S_K(t))^2\mid t\right]
 \le C_{\epsilon}\left\{b+u\bigl[\sigma^2+(t-\vartheta)^2\bigr]\right\},
\tag{9}
\]
...
\tag{10}
\]
Clipping \(t+\widehat S_K(t)\) to \([-1,1]\) cannot increase squared error because \(\vartheta\in[-1,1]\).  Combining \((7)\)--\((10)\) and using \(u<1\) would give that estimator normalized risk
\[
 C_{\epsilon}\{b+\sigma^2u+u(b+\sigma^2)\}
 \le C_{\epsilon}(b+\sigma^2u).
\]
After restoring the factor \(M^2\), this is exactly the proved product converse benchmark.  The unresolved statement is \((9)\): the existing linear-mark covariance lemma neither contains its centered mixed-arm kernel nor proves its exact-homogeneity degeneracy.  No inference claim and no adaptation to unknown \(M\) or \(\sigma\) is used.