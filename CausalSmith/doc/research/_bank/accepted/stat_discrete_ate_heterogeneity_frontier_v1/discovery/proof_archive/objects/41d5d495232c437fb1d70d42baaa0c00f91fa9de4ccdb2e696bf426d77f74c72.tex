...
\tag{13a}
\]
so the benchmark ratio is bounded.  Equations (9)--(13a) show that a sequence along which the upper benchmark divided by the lower benchmark diverges can have none of the following properties along an infinite subsequence: \(s\) bounded away from zero, \(x\) bounded away from zero, \(u\ge1\), \(u=O(b)\), or \(s=O(b)\).  Therefore such a sequence must satisfy
\[
b\ll u\ll1,
\qquad b\ll s\ll1.
\tag{14}
\]
Conversely, (14) is the only regime not covered by the preceding constant-factor comparisons; this does not assert that divergence occurs at every point of the wedge.  Since \(L=o(\sqrt n)\), the relations \(u\gg b\) and \(u\ll1\) are, up to boundary constants, equivalent to
\[
\sqrt n\,L\ll d\ll nL.
\tag{15}
\]
Indeed, \(u/(1/n)=d^2/(nL^2)\), while \(u/(d/n^2)=d/L^2\), and the first divergence in (15) implies the second.
...
The endpoint \(\sigma=0\) is covered by (12), every fixed positive radius by (9), saturation by (10), the small-alphabet and first parametric elbows by (11), and the shrinking-radius parametric elbow by (12).  Thus (14) is exactly the residual regime not decided by the established selector and radius channel. \(\square\)