...
Because \(L\geq4\), the explicit winsorized, Gram-stabilized estimator is covered by \(\mathrm{prop:cty\text{-}a1\text{-}a2\text{-}winsorized\text{-}expected\text{-}outer\text{-}upper}\).  It is a member of \(\mathcal T_n^{12,\pm}\).  Evaluating the infimum in \(\mathrm{def:cty\text{-}a1\text{-}a2\text{-}outer\text{-}risk}\) at this single law-independent rule gives, for all sufficiently large \(n\),
\[
 \begin{split}
 R_n^{12,\pm}(p,\nu,L)
 ...
 \end{split}
\]
for a finite constant \(C_0=C_0(p,\nu,L)\).  Since \(a_n>0\), division by \(a_n\) and passage to the upper limit yield
\[
 \limsup_{n\to\infty}{R_n^{12,\pm}(p,\nu,L)\over a_n}\leq C_0<\infty.
\]
For every real sequence, its lower limit does not exceed its upper limit.  Taking \(c=c_0/2\) and \(C=\max\{C_0,c\}\) therefore proves
\[
