...
\[
 w=A\Delta^{1/q}.
\]
Place \(x_j=(s_j,0)\) on \([-1/2,1/2]\times\{0\}\) with separation at least \(4w\).  Then \(M\asymp w^{-1}\geq c\Delta^{-1/q}\), and the disks \(S_j=B_2(x_j,w)\) are disjoint, lie strictly inside \(\mathcal X=[-3,3]^2\), and avoid the corners of \(\mathcal B=\partial([-1,1]\times[0,2])\).
...
The density perturbation vanishes for \(r\leq C\Delta\) and smoothly before \(r=w\).  The choice \(A>2C\) ensures \(2C\Delta<w\) also when \(q=1\).  Whenever the perturbation is nonzero,
\[
...
\]
The integral is positive for \(v\ne0\), including \(p=0\).  Compactness of the unit sphere gives a positive uniform minimum.  Enlarging \(L_0(p)\) verifies the Gram bound.  The Euclidean-ball VC clause follows from \(\mathrm{lem:euclidean\text{-}balls\text{-}vc}\).  Thus every displayed class clause holds.
...
\]
Thus the conditional laws agree outside \(0<r<2C\Delta\), an interval contained in \((0,w)\).  Since all Bernoulli probabilities lie in \([1/4,3/4]\), in either orientation
\[
...
\]
All constants are uniform in the asserted indices.  This proves the lemma for every \(p\geq0\).