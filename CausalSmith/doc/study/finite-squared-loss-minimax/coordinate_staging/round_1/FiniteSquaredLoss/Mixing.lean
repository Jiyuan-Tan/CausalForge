...
/-- With [nonnegative likelihood coefficients](hyp:hP), [an ordered action interval](hyp:hlu),
...
/-- With [nonnegative likelihood coefficients](hyp:hP) and [an ordered action interval](hyp:hlu),
...
/-- Every risk vector attained by feasible ambient coordinates [is dominated by the risk
of the associated ordinary finite procedure](goal). -/
...
/-- The convex hull of the attainable finite risk vectors [is compact](goal). -/
