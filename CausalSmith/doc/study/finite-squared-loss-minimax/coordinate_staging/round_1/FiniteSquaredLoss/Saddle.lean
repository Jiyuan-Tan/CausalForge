...
/-- For nonempty finite state and design spaces and [an ordered action interval](hyp:hlu),
the convexified risk game [has an attaining saddle point consisting of a risk vector and
a finite prior](goal). -/
...
/-- Given [nonnegative likelihood coefficients](hyp:hP), [an ordered action interval](hyp:hlu),
...
spaces, [an ordered real action interval](hyp:hlu) and [nonnegative likelihood coefficients](hyp:hP)
