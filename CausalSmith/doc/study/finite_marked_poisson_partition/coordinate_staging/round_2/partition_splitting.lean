import Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.Partition.Core
import Mathlib.Data.Finset.Sort
import Mathlib.Data.Nat.Choose.Multinomial
import Mathlib.GroupTheory.Perm.DomMulAct

/-!
# Poisson splitting across a finite measurable partition

This file proves that restricting a finite marked Poisson sample to the cells
of a finite measurable partition produces the product of the corresponding
independent cell experiments. It also records the resulting cell-count and
fixed-count-fibre laws.
-/
...
private lemma histogramWords_card (n : ℕ) (c : ι → ℕ)
  ...
  simp only [Finset.sum_const_zero, Finset.sum_filter, Finset.mem_univ,
    ↓reduceIte, Finset.prod_filter, true_and] at hmulti
  rw [hc] at hmulti
  rw [← hcardH]
...
private lemma fixedSizeEmbed_preimage_partitionCountFiber
  ...
  · intro hz
    ...
    · simp only [histogramWords, Finset.mem_filter, Finset.mem_univ, true_and]
      ...
      simpa [wordHistogram, cellIndices, w, fixedSizeEmbed] using hj
...
private lemma pi_cellLaws_restrict_partitionCountFiber
  ...
  congr 1
  symm
  simpa only [fixedPartitionEmbed] using Measure.pi_map_pi
...
private lemma poisson_multinomial_coefficient
  ...
  simp_rw [PMF.toMeasure_apply_singleton (poissonPMF _)
    _ (MeasurableSet.singleton _)]
  rw [hcard]
