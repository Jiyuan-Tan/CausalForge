  ...
  calc
    ∑' n : ℕ, (finiteSampleKernel P n) s * poissonMeasure lam {n}
        = ∑' n : ℕ,
          ...
          simpa only [finiteSampleKernel, smul_eq_mul] using
            (mul_comm ((finiteSampleKernel P n) s) (poissonMeasure lam {n}))
...
private lemma poisson_lintegral_natCast (r : ℝ≥0) :
  ...
  change (∑' n : ℕ, (n : ℝ≥0∞) * (poissonPMF r).toMeasure {n}) = _
  simp_rw [PMF.toMeasure_apply_singleton (poissonPMF r) _ (MeasurableSet.singleton _)]
...
private lemma poissonMeasure_singleton_one_ne_zero {t : ℝ≥0} (ht : t ≠ 0) :
  ...
  rw [show poissonMeasure t ({1} : Set ℕ) = poissonPMF t 1 by
    exact PMF.toMeasure_apply_singleton (poissonPMF t) 1 (MeasurableSet.singleton 1)]
...
private lemma poissonMeasure_zero : poissonMeasure 0 = Measure.dirac 0 := by
  ext s hs
  rw [poissonMeasure, PMF.toMeasure_apply _ hs, Measure.dirac_apply' 0 hs,
    tsum_eq_zero_add' ENNReal.summable]
  have hzero : poissonPMF 0 0 = 1 := by
    change ENNReal.ofReal (poissonPMFReal 0 0) = 1
    simp [poissonPMFReal]
  have hsucc (n : ℕ) : poissonPMF 0 (n + 1) = 0 := by
    change ENNReal.ofReal (poissonPMFReal 0 (n + 1)) = 0
    simp [poissonPMFReal]
  have hhead : s.indicator (poissonPMF 0) 0 = s.indicator 1 0 := by
    by_cases h0 : 0 ∈ s <;> simp [Set.indicator, h0, hzero]
  have htail : (∑' b : ℕ, s.indicator (poissonPMF 0) (b + 1)) = 0 := by
    have hterm : (fun b : ℕ ↦ s.indicator (poissonPMF 0) (b + 1)) =
        (fun _ ↦ 0) := by
      funext b
      by_cases hb : b + 1 ∈ s <;> simp [Set.indicator, hb, hsucc]
    rw [hterm, tsum_zero]
...
/-- For equal-mass finite intensity measures and common independent marks, the
KL divergence of the marked Poisson experiments is `lam` times the
finite-measure KL divergence. -/
