...
/-- If [the state map is measurable](hyp:hf) and [the loss is nonnegative in every state]
(hyp:hloss), then [the prior average of the composed loss is no greater than its finite
...
/-- With at least one available action, [a measurable deterministic state map]
(hyp:hf), and [nonnegative statewise loss](hyp:hloss), [the deterministic Bayes risk is at most
