...
theorem boundedRadialPolynomialOn_hasPolynomialL2Cover
  ...
  have hradial (k : Fin (p + 1)) :
    ...
    simpa [radialMonomialOn, radialMonomialClass, M] using hpull
...
theorem radialResidualScore_hasPolynomialL2Cover
  ...
  | inr hA =>
      ...
      change HasPolynomialL2Cover
        (radialResidualScore d p loc arm response q a b B j)
        (M * (R + P))
      simpa only [radialResidualScore, boundedRadialPolynomialOn,
        one_mul, sub_eq_add_neg] using hpull
...
theorem radialResidualScore_hasUniformPolynomialL2CoverWith
  ...
  have hradial :
    ...
    simpa [radialMonomialOn, radialMonomialClass, M] using
      radialMonomialOn_hasPolynomialL2CoverWith d p s.loc s.loc_measurable
  ...
  let s : S :=
    { loc := loc
      ...
      R_pos := hR }
  simpa only [radialResidualScore, boundedRadialPolynomialOn,
    radialResidualScoreEnvelope, M, P, one_mul, sub_eq_add_neg, s] using hpull s
