...
/-- A deliberately coarse uniform VC bound for a Boolean formula in `m`
independently parameterized classes of VC dimension at most `d`. -/
def booleanCombinationVCBound (m d : ℕ) : ℕ :=
  2 ^ (m * (d + 1) + 1)

/-- A deliberately coarse uniform VC bound for a union of `m` classes, each
of VC dimension at most `d`. -/
...
theorem HasVCAtMost.compDomain
  ...
  intro n S
  simpa only using hπ n (fun j => g (S j))
...
theorem HasPseudoDimAtMost.compDomain
  ...
  intro n T
  simpa only [subgraphClassifier] using
    hF n (fun j => (g (T j).1, (T j).2))

/-- The homogeneous linear threshold class generated by a finite feature
vector labels a point according to the sign of its parameter-feature pairing. -/
