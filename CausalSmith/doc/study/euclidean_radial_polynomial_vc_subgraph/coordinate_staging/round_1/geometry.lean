...
/-- This is an explicit upper bound on the pseudo-dimension of radial
monomials of one fixed degree whose Euclidean center may move. -/
def fixedRadialPseudoDimBound (d : ℕ) : ℕ :=
  booleanCombinationVCBound 3 (d + 2)

/-- This is an explicit upper bound on the pseudo-dimension when both the
Euclidean center and any radial degree up to a fixed cutoff may vary. -/
...
/-- A fixed-degree radial-monomial class with a moving Euclidean center has
pseudo-dimension bounded solely by the Euclidean dimension when its bandwidth
is positive and its annulus is ordered with nonnegative endpoints. -/
...
/-- Moving Euclidean centers together with all radial degrees up to a fixed
cutoff form a class with an explicit finite pseudo-dimension bound when the
bandwidth is positive and the annulus has ordered nonnegative endpoints. -/
theorem radialMonomialClass_hasPseudoDimAtMost
  ...
  simpa only [radialPseudoDimBound, Fintype.card_fin, pi, radialMonomialClass] using hr
