...
system.  Under an explicit Hahn pathwise-derivative hypothesis and an explicit
nonparametric tangent-space hypothesis, it proves that the AIPW score is a
pathwise gradient of the ATE functional and is the canonical gradient in the
