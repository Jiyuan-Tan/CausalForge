...
The law class below is the displayed Euclidean, uniform-kernel specialization of the CTY Assumptions 1--2 boundary design. Its clauses play three roles. The sampling, support, density, smoothness, variance, and moment-envelope clauses impose the standard random-design and local-polynomial regularity used to control bias and stochastic variation. The metric, kernel, and VC clause supplies a uniform set-indexed complexity bound of the kind used in empirical-process arguments \citep{Vapnik1971,Pollard1984,VanderVaart1996}. The interface, Gram, arm-mass, and distance-slice clauses encode the substantive geometric restrictions that make signed-distance local fitting well conditioned along the whole boundary.
...
The potential outcomes \leanref{sym:Y(t)}{$Y(t)$} and deterministic treatment \leanref{sym:T}{$T$} in \cref{obj:def:cty-a1-a2-class} give the boundary target its causal RD interpretation. The arm-specific regressions \leanref{sym:\mu_{t,P}}{$\mu_{t,P}$} are smooth enough for a degree-\(p\) signed-distance approximation, and the arm-specific variances \leanref{sym:\sigma^2_{t,P}}{$\sigma^2_{t,P}$} and \(2+\nu\) moment envelope keep the outcome scale uniform. Geometrically, the class admits compact rectifiable interfaces of controlled length inside rectangular supports and requires sufficient local mass from both arms at every interface point. These restrictions accommodate straight and curved interfaces with enough two-sided local design support, while excluding cusps that collapse one side of the local window, tangential contacts that make the signed-distance Gram nearly singular, and boundary pieces lying on the edge of the covariate support. The fixed rectangular hard family used later has corners on its treated rectangle; the certificate in \cref{obj:lem:cty-a1-a2-rectangle-angular-hypercube-all-orders} verifies that those corner points still satisfy the Gram and arm-mass thresholds in this law class.
