// PORTED from `site/src/lib/leanStatement.ts` (structuring only; the site's
// linkify/library helpers are omitted). The site renders rows FROM the emitted
// artifact, so this copy only has to agree with itself: if the two drift, the
// site still draws what P4 assigned. Re-sync by re-copying the source file and
// re-stripping the `library.js` import plus `linkifyStructured`/`structureAndLinkify`.
