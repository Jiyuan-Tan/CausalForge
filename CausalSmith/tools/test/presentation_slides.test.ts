...
import { extractDeckMarkdown, extractDsl } from "../src/presentation/stages/p6_slides.js";
...
describe("lintSlides", () => {
  ...
  it("flags unknown obj_ids, dropped theorems, displayed math, and empty headlines", () => {
    const doc = parseSlidesMd(deck(
      "## Results\n@formal thm:nope\n@informal thm:main:\nAnd \\[x=1\\] inline-displayed.",
    ));
    const gates = lintSlides(doc, BLOCKS).map((p) => p.gate).sort();
    expect(gates).toEqual([
      "slides-displayed-math",
